...
def load_data(path):
    ...
    # Split the DataFrame into 8 subsets
    subsets = []
    for i in range(8):
        start = i * subset_size
        end = (i + 1) * subset_size
        subset = df[start:end]
        subsets.append(subset.to_numpy())
    return subsets


def draw_zine_page(c, start_x, start_y, page_w, page_h, step, subset):
    txt_x = start_x + 15
    txt_y = start_y-step
    ...
    c.line(v_line_x, start_y, v_line_x, start_y - page_h)
    title_size = 6
    offset = 4
    c.setFont("Helvetica-Bold", title_size)
    for sents in subset:
        ...
        sent_de = sents[0]
        y_ = txt_y + offset
        c.drawString(txt_x, y_, sent_de)
        y_ -= step
        c.drawString(txt_x, y_, sent_it)
        txt_y -= 2*step

        line_y -= 2*step
        c.line(start_x, line_y, start_x + page_w, line_y)
...
def draw_zine(c):
    ...
    #              #
    ################
    # Draw 3 vertical lines with equal spacing
    vertical_spacing = width / 4  # Divide the page into 4 equal parts
    for j in range(1, 4):
        c.setLineWidth(2)
        x = j * vertical_spacing
        c.line(x, 0, x, height)

    path_df = 'data/german-italian-sentences.tsv'
    sent_set = load_data(path_df)
    start_x = 0
    start_y = height/2
    step = height/2/20
    for i in range(0, 4):
        draw_zine_page(c, start_x, start_y, width/4, height/2, step, sent_set[i])
        start_x += width/4
